...
std::shared_ptr<class Font> FontLoader::Create(const std::string& fontPath, float fontSize, int width, int height)
{
    ...
    if (fontStream.fail())
    {
        LOG_ERROR("Failed to load font [{}] - Unable to read buffer!", fontPath);
        return nullptr;
    }
    ...
    stbtt_BakeFontBitmap(buffer.data(), 0, fontSize, bitmap.data(), 
        width, height, 32, 96, (stbtt_bakedchar*)(data));
...
{
    return std::shared_ptr<class Font>();
}
