...
void StateStack::Pop()
{
    if (m_States.empty())
    {
        LOG_ERROR("Trying to pop an empty state stack!");
        return;
    }
...
void StateStack::Update(const float dt)
{
    ...
    if (topState.on_update.valid())
    {
        ...
        {
            LOG_ERROR("Failed to update top state: {}", error.what());
            return;
        }
        catch (const std::exception& ex)
        {
            LOG_ERROR("Failed to update top state: {}", ex.what());
            return;
        }
        catch (...)
        {
            LOG_ERROR("Failed to update top state: Unknown Error");
            return;
        }
    }
    ...
    if (topState.handle_inputs.valid())
    {
        ...
        {
            LOG_ERROR("Failed to handle top state's inputs: {}", error.what());
            return;
        }
    }
    ...
    if (topState.bKillState)
    {
        if (topState.on_exit.valid())
        {
            ...
            {
                LOG_ERROR("Failed to exit top state: {}", error.what());
                return;
            }
        }
...
void StateStack::Render()
{
    for (const auto& state : m_States)
    {
        if (state.on_render.valid())
        {
            ...
            {
                LOG_ERROR("Failed to render state [{}]: {}", state.name, error.what());
            }
        }
    }
}
