...
StateMachine::StateMachine( const sol::table& stateFuncs )
	: m_mapStates{}, m_sCurrentState{ "" }, m_StateTable{ stateFuncs }
{
}
...
void StateMachine::ChangeState( const std::string& stateName, bool bRemoveState, const sol::object& enterParams )
{
	...
	*/
	...
	{
		LOG_ERROR( "Failed to change states. [{}] does not exist.", stateName );
		return;
	}
	...
	else
	{
		...
		if ( oldState->on_exit.valid() )
		{
			...
			{
				LOG_ERROR( "Failed to exit state: {}", error.what() );
				return;
			}
		}
...
void StateMachine::ExitState()
{
	...
	if ( stateItr == m_mapStates.end() )
	{
		LOG_ERROR( "Failed to exit state: {} -- State does not exist.", m_sCurrentState );
		return;
	}
