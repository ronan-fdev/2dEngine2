...
void CrashLogger::Initialize()
{
	if ( m_bInitialized )
	{
		LOG_ERROR( "Failed to initialize CrashLogger - Has already been initialized." );
		return;
	}
	...
	LOG_INFO( "Scion Crash Logger has been initialized." );
}
...
void CrashLogger::SetProjectPath( const std::string& sProjectPath )
{
	...
	{
		LOG_ERROR( "Failed to set project path. {}", ec.message() );
	...
	if ( !fs::exists( fs::path{ sLogPath } ) )
	{
		...
		{
			LOG_ERROR( "Failed to create crash logs directory.", ec.message() );
			m_sProjectPath.clear();
			return;
		}
	}
...
void CrashLogger::CrashHandler( int signal )
{
	...
	assert( pLogFile && "Log File was not successfully created and opened." );

	// There is no file to log to, finish log write to console and exit
	if ( !pLogFile )
	{
		LOG_ERROR( "Failed to write to crash log file" );
