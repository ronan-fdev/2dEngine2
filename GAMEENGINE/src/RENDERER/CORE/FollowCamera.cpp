...
{
	assert( m_Params.scale > 0.f && "The scale must be greater than zero!" );
	...
	m_Camera.SetScale( m_Params.scale );
}
...
void FollowCamera::Update()
{
	...
	newCamPosition.x = ( transform.position.x - camWidth / ( 2.0f * m_Params.scale ) ) * m_Params.scale;
	newCamPosition.y = ( transform.position.y - camHeight / ( 2.0f * m_Params.scale ) ) * m_Params.scale;
...
void FollowCamera::CreateLuaFollowCamera( sol::state& lua, Registry& registry )
{
	...
	lua.new_usertype<FollowCamera>(
		...
		sol::call_constructor,
		sol::factories(
			[&]( const FollowCamParams& params, const Entity& entity ) {
