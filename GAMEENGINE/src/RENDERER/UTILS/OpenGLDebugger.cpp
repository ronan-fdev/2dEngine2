...
void OpenGLDebugger::init()
{
	...
	LOG_INFO( "Setting up OpenGL debug callback (requires OpenGL version > 4.3.)" );
...
#endif // __linux
			{
				LOG_WARN( "There is no valid OpenGL context bound." );
				s_init = false;
				return;
			}
	}
	...
	{ // Checking OpenGL version
		...
		if ( version < 43 )
		{
			LOG_WARN( "OpenGL version < 4.3...abort" );
			return;
		}
		else
		{
			LOG_INFO( "All good, OpenGL debug callback set properly" );
		}
	}
...
void OpenGLDebugger::setSeverityLevel( OpenGLDebuggerSeverity s )
{
	switch ( s )
	{
		case OpenGLDebuggerSeverity::Disable:
			s_settings.severity = OpenGLDebuggerSeverity::Disable;
			break;
		case OpenGLDebuggerSeverity::Notification:
			s_settings.severity = OpenGLDebuggerSeverity::Notification;
			break;
		case OpenGLDebuggerSeverity::Low:
			s_settings.severity = OpenGLDebuggerSeverity::Low;
			break;
		case OpenGLDebuggerSeverity::Medium:
			s_settings.severity = OpenGLDebuggerSeverity::Medium;
			break;
		case OpenGLDebuggerSeverity::High:
			s_settings.severity = OpenGLDebuggerSeverity::High;
			break;
		default:
			break;
	}
	...
	const char* p_message, const void* p_userParam )
{
	...
	{
	case GL_DEBUG_SEVERITY_HIGH:
		Logger::GetInstance().Warn(ss.str()); //Has to be changed to error.
