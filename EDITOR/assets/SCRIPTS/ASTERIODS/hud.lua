...
function Hud:Create()
	...
	for i = 0, gData:MaxLives()-1 do
		...
		transform.position = vec2(33*i,0)
...
function Hud:SetNumberPositions()
	...
	transform2.position.x = transform1.position.x + (16 * transform2.scale.x)
	local transform3 = Entity(self.m_HundredID):get_component(Transform)
	transform3.position.x = transform2.position.x + (16 * transform3.scale.x)
	local transform4 = Entity(self.m_TensID):get_component(Transform)
	transform4.position.x = transform3.position.x + (16 * transform4.scale.x)
	local transform5 = Entity(self.m_OnesID):get_component(Transform)
	transform5.position.x = transform4.position.x + (16 * transform5.scale.x)
end
...
function Hud:SetScore(current_score)
	...
	local teens = GetDigit(current_score,2)
